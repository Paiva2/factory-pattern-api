    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
            .authorizeHttpRequests(req -> {
                ...
                req.antMatchers(HttpMethod.POST, "/api/v1/user/forgot-password").permitAll();
                ...
                req.antMatchers(HttpMethod.POST, "/api/v1/playlist/copy/{playlistId}").hasAnyRole("USER", "ADMIN");
                ...
                req.antMatchers(HttpMethod.POST, "/api/v1/favourite/new/{musicId}").hasAnyRole("USER", "ADMIN");
                ...
                req.antMatchers(HttpMethod.GET, "/api/v1/favourite/list").hasAnyRole("USER", "ADMIN");
                req.antMatchers(HttpMethod.GET, "/api/v1/playlist/{playlistId}").permitAll();
                req.antMatchers(HttpMethod.GET, "/api/v1/playlist/export/{playlistId}").permitAll();

                req.antMatchers(HttpMethod.PATCH, "/api/v1/music/{musicId}").hasAnyRole("MUSICIAN", "ADMIN");
